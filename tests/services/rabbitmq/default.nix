{
  stateful = true;
  paths = ["modules/services/dev/rabbitmq.nix"];
  checks = {
    config = {
      file = ./config.nix;
      covers = ["configuration"];
      detail = "Independent enable/disable, parent override, custom listener, invalid port, exposure and card/proxy selection, native state/cookie backup and runtime credential/bootstrap wiring with password input over stdin.";
    };
    recovery = {
      file = ./recovery.nix;
