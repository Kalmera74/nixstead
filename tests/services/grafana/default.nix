{
  stateful = true;
  paths = ["modules/services/dev/grafana.nix" "modules/services/dev/dashboards/"];
  checks = {
    config = {
      file = ./config.nix;
      covers = ["configuration"];
      detail = "Independent enable/disable, parent override, custom listener, invalid port, exposure and card/proxy selection, plus native application settings, encrypted-source credential paths and essential SQLite backup wiring.";
    };
    recovery = {
      file = ./recovery.nix;
