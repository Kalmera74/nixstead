...
nix develop --command bash scripts/check-source.sh
nix fmt -- --fail-on-change
nix build -L .#checks.x86_64-linux.public-module-api .#checks.x86_64-linux.service-docs
nix build --no-link -L path:.#checks.x86_64-linux.service-config path:.#checks.x86_64-linux.test-catalogue
```
...
The automatic GitHub Actions workflow keeps PR and main-branch validation
lightweight. It evaluates every discovered repository host, then dry-runs the
main CLI and setup packages, public-module API, Python check, and
documentation/catalogue checks. It also verifies formatting and runs ShellCheck
on x86_64. It does not build those derivations or evaluate the exhaustive
service and runtime VM matrix. A blanket `nix flake check --no-build` is
deliberately avoided because Nix still evaluates every derivation exposed under
`checks`.
...
[generated test coverage catalogue](generated/test-coverage.md). Configuration,
Python, source and documentation builds run in the manually dispatched heavy
workflow. The packaged Python check supplies real Redis, SQLite and credential
tools, avoiding dependency-based skips from a narrower development shell.
