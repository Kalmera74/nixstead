...
| Pi-hole DNS adapter startup | `service-pihole-config` on both architectures, including explicit default-off mutation | Runtime smoke passed in 63.91s on x86_64-linux | The shipped opt-in sync starts, authenticates to a bounded Pi-hole v6 API peer and applies the configured owned records once. Idempotence, credential rotation, failure handling, reboot, live-version compatibility, appliance recovery, DNS queries and ARM runtime remain unverified |
